...
# ╔═╡ 501f38b0-37bf-11f0-1d2f-41960aa00c7e
begin
	using CSV, DataFrames, Flux,  Flux.Losses, Statistics, Random
	using MLUtils: stratifiedobs
	using MLDataPattern: paddedview
	using MLLabelUtils: labelmap, inversemap
	using Transformers
	using Zygote
	using ROCAnalysis, Plots
	using Flux: onehotbatch, onecold, sigmoid
	import Optimisers
	using Optimisers: ADAM
end

# ╔═╡ 53aeabea-f604-4705-9d75-ffaf7ddc5203
md"# Prepare the Environment"

# ╔═╡ c6ae224e-0f32-4949-88dd-3ef1d4a06b26
md" #  Load datasets"

# ╔═╡ 5ea51276-afff-4169-b0ef-29e2817d6179
begin
	labels_df = CSV.read("C:/Users/s.ndongo/OneDrive - I-TECH Namibia/NUST/TAI911S/Assignment 3/medical_tc_labels.csv", DataFrame)
	#label_map = Dict(row.condition_label => row.condition_name for row in eachrow(labels_df))
	
	train_df = CSV.read("C:/Users/s.ndongo/OneDrive - I-TECH Namibia/NUST/TAI911S/Assignment 3/medical_tc_train.csv", DataFrame)
	test_df = CSV.read("C:/Users/s.ndongo/OneDrive - I-TECH Namibia/NUST/TAI911S/Assignment 3/medical_tc_test.csv", DataFrame)
end

# ╔═╡ 73cba3b5-6564-4f5e-af1f-6950c9655751
begin
	using StatsPlots
	# Group and count the data
	summary_df = combine(groupby(train_df, :condition_label), nrow => :count)
	summary_df2 = combine(groupby(test_df, :condition_label), nrow => :count)

	# Prepare x and y values for each plot
    x_vals = string.(summary_df.condition_label)
    y_vals = summary_df.count

	X_vals = string.(summary_df2.condition_label)
    Y_vals = summary_df2.count

    # Compute annotation positions
    annotations = [(i -0.5, y_vals[i] -140, text(string(y_vals[i]), :White, :center, 13)) 
		for i in 1:length(x_vals)]

	# Plot using a regular bar plot
	
	# First plot
	p1 = bar(x_vals, y_vals,
    	title = "Train Data: Condition Distribution", legend = false,
   		xlabel = "Condition",
    	ylabel = "Count",
	...
	# Compute annotation positions
    annots = [(i -0.5, Y_vals[i], text(string(Y_vals[i]), :black, :center, 13)) 
		for i in 1:length(X_vals)]
	
    # Second plot
	p2 = bar(X_vals, Y_vals,
    	title = "Test Data: Condition Distribution", legend = false,
   		xlabel = "Condition",
    	ylabel = "Count",
	    annotations = annots,
		color = :orange
...
# ╔═╡ b8ae7085-f345-402b-a4d1-0b1eac926a75
for col_names in names(labels_df)
	println(col_names)
end

# ╔═╡ 89852240-0a44-449b-8a08-b1d6cbd1e0a8
for col_names in names(train_df)
	println(col_names)
end

# ╔═╡ ad00086c-b3a5-425e-803a-9f8eb164b85f
for col_names in names(test_df)
	println(col_names)
end

# ╔═╡ 652221af-0109-4501-b523-52aaf7a57ae0
md"### Extract inputs and labels"

# ╔═╡ 0f244775-2971-4042-8efc-b0bebc8574d8
begin
	#train_labels = [label_map[lbl] for lbl in train_df.condition_label]
	#test_labels = [label_map[lbl] for lbl in test_df.condition_label]
	
	train_texts = train_df.medical_abstract
	train_labels = train_df.condition_label
	
	test_texts = test_df[!, :medical_abstract]
	test_labels = test_df.condition_label	
end

# ╔═╡ f927b956-8aa8-4476-b3fc-86ee66c8d3f2
begin
	classes = sort(unique(train_labels))
	lbl_map = Dict(c => i for (i, c) in enumerate(classes))
	#lbl_map = labelmap(train_labels)
	#lbl_map = Dict("neoplasms" => 1, "digestive system diseases" => 2, "nervous system diseases" => 3, "cardiovascular diseases"=> 4, "general pathological conditions" => 5)
	
	y_train = [lbl_map[l] for l in train_labels]
	y_test = [lbl_map[l] for l in test_labels]
	num_classes = length(unique(y_train))
end

# ╔═╡ 138421a6-d747-4490-9e10-4fe42be9276a
println("Label map: ", lbl_map)

# ╔═╡ 0e1ef39c-0745-464b-8ddd-5c75f5ead227
md"# Medical Abstracts Text Classification"

# ╔═╡ 378c0453-fd3f-4b4c-b7b6-3b5ddd02ecbe
md" # Tokenization, padding, and input preparation"

# ╔═╡ ed1a9a6d-d60e-4beb-87ca-f59fbc54c508
md"## Tokenize Text & Create Vocabulary"

# ╔═╡ 011aea68-fc8c-4b4a-a886-5bd05153a84d
begin
	function simple_tokenizer(text)
		split(lowercase(strip(text)))
	end

	# Build vocabulary
	tokenized = collect(simple_tokenizer.(train_texts))
	all_tokens = reduce(vcat, tokenized)
	vocab = Dict(word => i for (i, word) in enumerate(unique(all_tokens)))
	vocab_size = length(vocab) + 1  # for unknowns
end

# ╔═╡ a5c3992f-27d6-4b59-877f-33be52ff2734
md"## Convert Texts to Sequences"

# ╔═╡ e9205a65-5eca-4c62-b58c-9994701b5c98
begin
	max_len = 100
	
	# Define padding function
	function pad_sequence(seq, len; pad_val=0)
		length(seq) ≥ len ? seq[1:len] : vcat(seq, fill(pad_val, len - length(seq)))
	end

	# Convert text to padded integer sequence
	function texts_to_seqs(texts)
		[pad_sequence([get(vocab, w, vocab_size) for w in simple_tokenizer(t)], max_len) for t in texts]
	end

	# Apply to datasets
	X_train = texts_to_seqs(train_texts)
	X_test = texts_to_seqs(test_texts)
end

# ╔═╡ 421d3eec-79cd-4a99-91ed-192916436e63
# Convert data to matrix for batching
X_matrix = hcat(X_train)

# ╔═╡ 4297d387-bec8-42c6-96f3-bf7fab8f1ee5
size(X_matrix)

# ╔═╡ 188e23fb-29ae-43a3-b690-ad9da66a0785
md"#  Define and compile the LSTM model"

# ╔═╡ 83527300-952f-4596-8aa4-cf73436ac80d
begin
	# Hyperparameters 
	embedding_dim = 50
	hidden_dim = 64
...
# ╔═╡ 883aef25-5e16-4ea4-b389-2566cb2d654a
# Generate synthetic data
function generate_synthetic_data(n_samples, seq_length, vocab_size, output_dim)
    X = [rand(1:vocab_size, seq_length) for _ in 1:n_samples]  # Random sequences
    y = Flux.onehotbatch(rand(1:output_dim, n_samples), 1:output_dim)  # Random one-hot labels
    return X, y
end

# ╔═╡ ff560c09-88bd-4e17-8d1f-fc6946107943
# Loss function
loss(x, y) = Flux.crossentropy(dropdims(model(x), dims=2), y)

# ╔═╡ 480d40b7-35be-44bc-a81e-8113b774e787
# Optimizer
opt = ADAM(learning_rate)

# ╔═╡ 3c2363f3-8cf4-445b-82c8-c869086c1849
# Data loader
function data_loader(X, y, batch_size=32)
    n = size(X, 1)
    indices = shuffle(1:n)
    batches = []
...
# ╔═╡ 30e96c84-a0f9-47fa-9ccb-70cc400a50f8
md"# Training Loop"

# ╔═╡ 8c7ed334-9f86-461e-8e32-f30b7807b9b3
begin
	
	function train_model!(model, X, y, opt; epochs=10, batch_size=32)
	    ...
	    for epoch in 1:epochs
	        ...
	        for i in 1:batch_size:n
	            batch_idx = indices[i:min(i + batch_size - 1, n)]
	            batch_X = X[batch_idx, :]             # shape: (batch_size, 1)
	            batch_y = y[:, batch_idx]             # shape: (num_classes, batch_size)
	
	            # Convert Vector{Vector{Int64}} to matrix format (sequence_length, batch_size)
	            # Assuming each batch_X[i] is a Vector{Int64} representing a sequence
	            input_seqs = hcat(batch_X[:, 1]...)    # shape: (seq_len, batch_size)
	            ...
	            loss, grads = Flux.withgradient(model) do m
	                ŷ = m(input_seqs)
	                Flux.crossentropy(ŷ, batch_y)
	            end
	
	            Flux.Optimise.update!(opt, Flux.params(model), grads)
	
	            println("  Batch loss: $(round(loss, digits=4))")
	        end
	    end
	end
...
# ╔═╡ d063a077-d45e-4a05-ab6b-169832d7f179
begin
	X_data1 = X_matrix
	y_data = Flux.onehotbatch(y_train, 1:num_classes)                      # True labels
end

# ╔═╡ 441b5d6e-3d3a-4b54-9f1d-b4d449273289
begin
	X_data_clean = [map(x -> max(x, 1), x[1]) for x in X_data1]  # replace 0s with 1s in each sequence
	X_data = reshape(X_data_clean, :, 1)  # ensure it's a matrix of shape (num_samples, 1)
end

# ╔═╡ 74a2af3d-cff2-4abd-a4cb-d6556f3fddb1
train_model!(model, X_data, y_data, opt, epochs=10, batch_size=32)

# ╔═╡ 1e291732-a25e-4f28-9e90-57536b0bfaf4
# Evaluate on a small test batch
test_X = X_matrix[:, 1:10]
test_y = y[:, 1:10]
Flux.reset!(model)
test_loss = loss(test_X, test_y)
println("Test loss: $test_loss")

# ╔═╡ 6f9f1687-6383-4509-b8e8-1ed10bb96d9d
md"# Training loop"

# ╔═╡ 9449b661-73b6-4759-b3a1-d3f3f64ce7c4
# Training loop
for row in eachrow(train_df)
    x = row.text
    y = row.condition_label
    y_oh = OneHotVector(y, num_classes)

    grads = gradient(() -> loss(x, y_oh), Flux.params(model))
    model, state = Optimisers.update(opt, model, grads, state)
end

# ╔═╡ 009de6b7-ceec-4c19-84a3-93012ba20429
# Training loop
for epoch in 1:10  # number of epochs
    println("Epoch $epoch")

    for (x, y) in zip(train_texts, train_labels)
        # If y is a class index, one-hot encode it:
        y_vec = Flux.onehot(y, 1:num_classes)
...
# ╔═╡ 000d0e25-8b26-4e9f-8067-eacfb2e558ae
for (x_str, y_vec) in zip(train_texts, train_labels_onehot)
    x = preprocess(x_str)  # convert String -> Vector{Int}
    gs = gradient(ps) do
        loss(x, y_vec)     # now both arguments match the method signature
    end
    Flux.Optimise.update!(opt, ps, gs)
end


# ╔═╡ 005649d5-9bc6-442f-8e7f-46004e73e83f
md"#  Predictions & evaluation metrics"

# ╔═╡ 4c38a901-fb44-421e-b7a2-d9a80f1395cb
begin
	function predict_probs(model, X)
		[model(Flux.unsqueeze(hcat(x...), 2))[1] for x in X]
	end

	y_probs = predict_probs(model, X_test)
	y_pred = map(p -> p > 0.5 ? 1 : 0, y_probs)
...
# ╔═╡ 7231e549-3cfd-4f62-86f4-98a5e1ad4570
md"# ROC curve and AUC"

# ╔═╡ 9d05f0f8-a8bf-4a05-8847-7d01db0a9050
begin
	roc = roc_curve(test_labels, y_probs)
	auc_val = auc(roc)

	plot(roc, title = "ROC Curve", label = "AUC = $(round(auc_val, digits=4))", linewidth = 2)
end
...
manifest_format = "2.0"
project_hash = "1a5d22033a62b45ed474049aa4d763a83326ff85"
...
[[deps.Fetch]]
deps = ["Base64", "HTTP", "JSON3", "Random", "StructTypes", "p7zip_jll"]
git-tree-sha1 = "781292162fd5bfe8d001210f9dddbb6baa509bf4"
...
[[deps.FuncTransforms]]
git-tree-sha1 = "04a8f9343d61ef7360c972c7aa2b978fdf7569bb"
uuid = "79abecb7-a74d-442d-bb0e-6136fbda6b73"
version = "0.1.1"
...
[[deps.HuggingFaceApi]]
deps = ["Dates", "Downloads", "JSON3", "LibGit2", "OhMyArtifacts", "Pkg", "SHA"]
git-tree-sha1 = "bcf9b0ee12839d9bbee389ec13cd926845a2d39f"
uuid = "3cc741c3-0c9d-4fbe-84fa-cdec264173de"
version = "0.1.0"
...
deps = ["Adapt", "ChainRulesCore", "FuncTransforms", "LinearAlgebra", "NNlib", "Random", "Static"]
git-tree-sha1 = "0a908d2d813ff5647c733cdd5f10a72dd6b326d3"
uuid = "12afc1b8-fad6-47e1-9132-84abc478905f"
version = "0.3.3"

    [deps.NeuralAttentionlib.extensions]
    ...
    [deps.NeuralAttentionlib.weakdeps]
    AMDGPU = "21141c5a-9bdb-4563-92ae-f87d6854732e"
    CUDA = "052768ef-5323-5732-b1bb-66c8b64840ba"
...
[[deps.ROCAnalysis]]
deps = ["DataFrames", "DelimitedFiles", "Distributions", "LinearAlgebra", "Printf", "Random", "RecipesBase", "SpecialFunctions"]
git-tree-sha1 = "c7574cbe36ddfe905acbaffea25f47b4b7ff73ed"
...
[[deps.Transformers]]
deps = ["BangBang", "Base64", "BytePairEncoding", "ChainRulesCore", "DataDeps", "DataStructures", "Dates", "DelimitedFiles", "DoubleArrayTries", "Fetch", "FillArrays", "Flux", "FuncPipelines", "Functors", "HTTP", "HuggingFaceApi", "InteractiveUtils", "JSON3", "LRUCache", "LightXML", "LinearAlgebra", "Mmap", "NNlib", "NeuralAttentionlib", "Pickle", "Pkg", "PrimitiveOneHot", "Random", "SHA", "SafeTensors", "Static", "Statistics", "StringViews", "StructWalk", "TextEncodeBase", "Tricks", "Unicode", "ValSplit", "WordTokenizers", "Zygote"]
git-tree-sha1 = "335cc90642726cee40c8db1bfbca6da99e59ebea"
uuid = "21ca0261-441d-5938-ace7-c90938fde4d4"
version = "0.3.1"

    [deps.Transformers.extensions]
    TransformersAMDGPUExt = "AMDGPU"
    TransformersCUDAExt = "CUDA"
    TransformersMetalExt = "Metal"

    [deps.Transformers.weakdeps]
    AMDGPU = "21141c5a-9bdb-4563-92ae-f87d6854732e"
    CUDA = "052768ef-5323-5732-b1bb-66c8b64840ba"
    Metal = "dde4c033-4e86-420c-a63e-0dd931031962"
...
[[deps.ValSplit]]
deps = ["ExprTools", "Tricks"]
git-tree-sha1 = "3e1d94627f9276c40034c80dc5ab29ac1a3b06c0"
uuid = "0625e100-946b-11ec-09cd-6328dd093154"
version = "0.1.1"
