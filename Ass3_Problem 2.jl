...
# ╔═╡ 323e71e0-4277-11f0-1abc-7901dfb0e768
begin
	using Flux
...
# ╔═╡ 014d5080-8cad-49d1-a36d-1103dcddcadb
# Data Preprocessing
function load_images(data_dir, img_size=(224, 224))
    classes = ["Arborio", "Basmati", "Ipsala", "Jasmine", "Karacadag"]
    images = []
    labels = []
    
    for (i, class) in enumerate(classes)
        class_dir = joinpath(data_dir, class)
        for img_file in readdir(class_dir)
            if endswith(img_file, ".jpg")
                img_path = joinpath(class_dir, img_file)
                img = load(img_path)
                img = imresize(img, img_size)
                img_array = channelview(RGB.(img))
                push!(images, permutedims(img_array, (2, 3, 1)))
                push!(labels, i)
            end
        end
    end
    ...
    labels = onehotbatch(labels, 1:length(classes))
    return images, labels, classes
end

# ╔═╡ b97cfaaf-5869-447a-81d6-6d89728ab3cf
# Normalize images (mean subtraction as per AlexNet)
function normalize_images(images)
    images = Float32.(images)
    mean_img = mean(images, dims=(1,2,4))
    images = images .- mean_img
    return images
end

# ╔═╡ c317f03a-a076-40db-a042-b9d72a222e52
# Data augmentation (translations and horizontal flips)
function augment_image(img)
    # Random crop (224x224 from 256x256)
    img = imresize(img, (256, 256))
    x_offset = rand(0:32)
    y_offset = rand(0:32)
    cropped = img[x_offset+1:x_offset+224, y_offset+1:y_offset+224, :]
    
    # Random horizontal flip
    if rand() > 0.5
        cropped = reverse(cropped, dims=2)
    end
    
    return cropped
end

# ╔═╡ 804413cc-770f-46c4-876e-a47333207f4c
# Split data into train and test sets
function train_test_split(images, labels; train_ratio=0.8)
    n = size(images)[end]
    indices = shuffle(1:n)
...
# ╔═╡ abc93372-8962-4f60-9917-c6da7a69232c
# Local Response Normalization (as per AlexNet)
function local_response_norm(x; k=2, n=5, α=1e-4, β=0.75)
    ...
    for b in 1:batch_size
        for c in 1:size(x, 3)
            for h in 1:size(x, 1)
                for w in 1:size(x, 2)
                    start_c = max(1, c - div(n, 2))
                    end_c = min(size(x, 3), c + div(n, 2))
                    sum_squares = sum(x[h, w, start_c:end_c, b].^2)
                    out[h, w, c, b] = x[h, w, c, b] / (k + α * sum_squares)^β
                end
            end
        end
    end
...
# ╔═╡ adcba94a-2fe1-466b-9c5c-343798f6c854
# AlexNet Architecture
function build_alexnet(num_classes)
    model = Chain(
        # Conv1
        Conv((11, 11), 3=>96, relu; stride=4, pad=2),
...
# ╔═╡ 081401b9-440e-4168-92b5-f592d01f9d00
# Evaluation Metrics
function evaluate_model(model, test_images, test_labels, classes)
    testmode!(model)
    preds = model(test_images)
    pred_labels = onecold(preds)
    true_labels = onecold(test_labels)
    ...
    for i in 1:length(classes)
        ...
        push!(sensitivity_scores, sensitivity)
    end
    
    return accuracy, precision_scores, recall_scores, sensitivity_scores, preds
end

# ╔═╡ f71757dd-8936-4c11-9c75-b7e0324b2da5
# ROC Curve and AUC
function plot_roc_curve(preds, test_labels, classes)
    roc_plot = plot(title="ROC Curves for Rice Classification", xlabel="False Positive Rate", ylabel="True Positive Rate", legend=:bottomright)
    auc_scores = Float64[]
    
    for i in 1:length(classes)
        scores = preds[i, :]
        labels = test_labels[i, :]
        roc = rocplot(scores, labels)
        auc_score = auc(roc)
        push!(auc_scores, auc_score)
        plot!(roc, label="$(classes[i]) (AUC = $(round(auc_score, digits=3)))")
    end
    
    return roc_plot, auc_scores
end

# ╔═╡ 690ff920-c3f3-437e-839a-6e7bfeab41f9
# Main function
function main(data_dir)
    ...
    model = build_alexnet(length(classes))
    ...
    for epoch in 1:90
        trainmode!(model)
        for i in 1:size(train_images)[end]
            img = train_images[:,:,:,i:i]
            img = augment_image(img[:,:,:,1])
            img = reshape(img, size(img, 1), size(img, 2), size(img, 3), 1)
            Flux.train!(loss, params(model), [(img, train_labels[:,i:i])], opt)
        end
        println("Epoch $epoch completed")
    end
    
    # Evaluate model
    accuracy, precision, recall, sensitivity, preds = evaluate_model(model, test_images, test_labels, classes)
    
    #–

    # Plot ROC curve
    roc_plot, auc_scores = plot_roc_curve(preds, test_labels, classes)
    for (i, class) in enumerate(classes)
        println("AUC for $class: $(auc_scores[i])")
    end
    display(roc_plot)
end

# ╔═╡ f641c4f5-de7f-447b-b0bd-1d52f8864fea
# Example usage
function load_and_prepare_dataset(data_dir)
    ...
    println("Training images: $(size(train_images))")
    println("Test images: $(size(test_images))")
    
    return (train_images, train_labels), (test_images, test_labels), classes
end

# ╔═╡ 9731d22c-deb5-436b-89d1-a952dce54e72
begin
    using FileIO
    println("Loading dataset...")
    (train_images, train_labels), (test_images, test_labels), classes = load_and_prepare_dataset("C:/Users/s.ndongo/Desktop/Rice_Image_Dataset")
...
manifest_format = "2.0"
project_hash = "8358eccf3654b26019158ebbc8678114cf684e57"
